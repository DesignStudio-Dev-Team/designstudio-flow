  ...
  >
    ...
    <div class="dsf-editor-help__card" :class="`dsf-editor-help__card--${step}`" :style="cardStyle">
      ...
      <template v-if="isTour">
        <p class="dsf-editor-help__eyebrow">Quick tour · {{ stepNumber }} of 9</p>
        <h2>{{ tour.title }}</h2>
        ...
        <div class="dsf-editor-help__actions">
          <button v-if="['page-settings-detail', 'theme-detail', 'preview', 'organize'].includes(step)" type="button" class="dsf-editor-help__button" @click="emit('next')">{{ nextLabel }}</button>
...
function syncViewport() {
  isMobileViewport.value = typeof window !== 'undefined' && window.innerWidth <= 620
}
const stepNumber = computed(() => ({ dock: 1, 'page-settings-detail': 2, theme: 3, 'theme-detail': 4, preview: 5, organize: 6, add: 7, settings: 8, background: 9 }[props.step] || 1))
const targetSelector = computed(() => ({
  ...
  preview: '.dsf-dock',
  organize: '[data-dsf-help="dock-help"]',
...
const cardStyle = computed(() => {
  ...
  if (isMobileViewport.value) {
    const dockStep = ['dock', 'theme', 'preview', 'organize', 'add'].includes(props.step)
    return dockStep
...
const nextLabel = computed(() => ({
  ...
  preview: 'Show quick actions',
  organize: 'Show me how to add a block',
}[props.step] || 'Continue'))
const waitingMessage = computed(() => ({
  dock: isMobileViewport.value && !props.mobileActionsOpen
    ? 'Tap the glowing menu button to reveal the editor actions.'
    : `${isMobileViewport.value ? 'Tap' : 'Click'} the glowing Page Settings icon to continue.`,
...
const tour = computed(() => ({
  dock: {
    title: 'Start with Page Settings',
    description: 'Click Page Settings to update page-level WordPress details such as its title, URL, status, and optional features.',
  },
  ...
  organize: {
    ...
    description: 'The last four icons are always close at hand while you build.',
    items: ['Help shows relevant tips and can restart this guide.', 'History restores an earlier saved version.', 'Structure lets you jump to, rename, and reorder your blocks.'],
  },
  add: {
...
watch([() => props.visible, () => props.step, () => props.mobileActionsOpen, isMobileViewport], queueMeasure, { immediate: true })
onMounted(() => {
  syncViewport()
...
.dsf-editor-help__card { position: fixed; right: 28px; bottom: 96px; width: min(350px, calc(100vw - 28px)); max-height: calc(100dvh - 100px); overflow: auto; padding: 20px; border: 1px solid rgb(12 95 168 / 20%); border-radius: 16px; background: #fff; color: #17202a; box-shadow: 0 22px 70px rgb(15 23 42 / 28%); pointer-events: auto; }
